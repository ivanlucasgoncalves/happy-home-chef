...
function teambooking_autoloader($class_name)
{
	...
	$pos = strpos($class_name, 'Stripe');
	if ($tbk_stripe_calling && FALSE !== $pos) {
		$class_name = substr_replace($class_name, '', $pos, strlen('Stripe'));
		$classes_dir     = realpath(TEAMBOOKING_PATH)
			. DIRECTORY_SEPARATOR . 'src'
			. DIRECTORY_SEPARATOR . 'TeamBooking'
			. DIRECTORY_SEPARATOR . 'PaymentGateways'
			. DIRECTORY_SEPARATOR . 'Stripe'
			. DIRECTORY_SEPARATOR . 'new_lib';
		$what_to_replace = array('_', '\\');
		$class_file      = str_replace($what_to_replace, DIRECTORY_SEPARATOR, $class_name) . '.php';
		$filePath        = $classes_dir . $class_file;
		if (file_exists($filePath)) {
