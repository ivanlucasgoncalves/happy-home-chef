...
class Google_Client
{
  ...
  public function __construct($config = null)
  {
    if (is_string($config) && strlen($config)) {
      ...
      if ($this->isAppEngine()) {
        ...
        $config->setCacheClass('TeamBooking\\Google\\Google_Cache_Memcache');
      }

      if (version_compare(phpversion(), "5.3.4", "<=") || $this->isAppEngine()) {
        // Automatically disable compress.zlib, as currently unsupported.
        $config->setClassConfig('TeamBooking\\Google\\Google_Http_Request', 'disable_gzip', true);
      }
    }
    ...
    if ($config->getIoClass() == Google_Config::USE_AUTO_IO_SELECTION) {
      if (function_exists('curl_version') && function_exists('curl_exec')
          && !$this->isAppEngine()) {
        $config->setIoClass("TeamBooking\\Google\\Google_IO_Curl");
      } else {
        $config->setIoClass("TeamBooking\\Google\\Google_IO_Stream");
      }
    }
   ...
   */
  public function verifySignedJwt($id_token, $cert_location, $audience, $issuers, $max_expiry = null)
  {
    $auth = new Google_Auth_OAuth2($this);
    $certs = $auth->retrieveCertsFromLocation($cert_location);
    return $auth->verifySignedJwtWithCerts($id_token, $certs, $audience, $issuers, $max_expiry);
  }
  ...
  public function execute($request)
  {
    if ($request instanceof Google_Http_Request) {
      ...
      if (!$this->getClassConfig("TeamBooking\\Google\\Google_Http_Request", "disable_gzip")) {
        $request->enableGzip();
      }
