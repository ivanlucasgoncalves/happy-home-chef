jQuery(document).ready(function ($) {
    ...
    $(document).on('click keydown', '.tbk-load-email-template, .tbk-save-email-template', function (e) {
        e.stopPropagation();
        if (e.which == 13 || e.which == 32 || e.which == 1) {
            var modal_id = $(this).data('modal-id');
            var $modal = $('#' + modal_id);
            if ($(this).hasClass('tbk-save-email-template')) {
                var content = $(this).closest('.tb-email-editor').find('.wp-editor-area').val();
                $modal.find('.tbk-scaled').html(content);
                ...
                $modal
                    .uiModal({
                        ...
                        onApprove : function ($clicked) {
                            ...
                            $clicked.addClass('tbk-loading');
                            var name = $modal.find('input[name="tbk-email-template-name"]').val();
                            var description = $modal.find('textarea[name="tbk-email-template-description"]').val();
                            $.post(
                                TB_vars.post_url,
                                {
                                    action              : 'tbk_add_email_template',
                                    _wpnonce            : TB_vars.wpNonce,
                                    template_name       : name,
    ...
    $(document).on('click keydown', '.tbk-email-templates li', function (e) {
        if (e.which == 13 || e.which == 32 || e.which == 1) {
            var $modal = $(this).closest('.tbk-modal');
            var modal_id = $modal.attr('id');
            var content = $(this).find('.tbk-scaled').html();
            $('[data-modal-id=' + modal_id + ']').closest('.tb-email-editor').find('.wp-editor-area').text(content);
            $modal.uiModal('hide');
        }
    });
    ...
    $(document).on('click keydown', '.tbk-reservation-details-modal .tbk-edit-reservation-data', function (e) {
        ...
        if (e.which == 13 || e.which == 32 || e.which == 1) {
            ...
            $modal.find('.tb-customer-details .tbk-toggle-on-edit').toggle();
        }
    });
    ...
    $(document).on('click keydown', '.tbk-reservation-details-modal .tbk-send-confirmation-again, .tbk-reservation-details-modal .tbk-send-cancellation-again', function (e) {
        e.stopPropagation();
        if (e.which == 13 || e.which == 32 || e.which == 1) {
            var $button = $(this);
            ...
            var id = $button.data('id'),
                type;
            if ($button.hasClass('tbk-send-confirmation-again')) {
                type = 'confirmation';
            } else {
                type = 'cancellation';
            }
    ...
    $(document).on('click keydown', '.tbk-edit-reservation-data-save', function () {
        var button = $(this);
        button.toggleClass('loading');
        var reservation_id = $(this).data('id');
        var fields = {};
        var lang = '';
        var $modal = button.closest(".ui.small.modal");
        $modal.find('.tb-customer-details input:not(.noedit)').each(function () {
            var key = $(this).data('key');
            fields[key] = $(this).val();
            $(this).prop('readonly', true);
        });
        $modal.find('.tbk-language-select').each(function () {
            lang = $(this).val();
        });
        $.post(
            TB_vars.post_url,
            ...
            function (response) {
                ...
                $modal.find('.tbk-reservation-lang-text').text($modal.find('.tbk-language-select').children(':selected').text());
            }
        );
    });

    $('.tb-flatpickr').each(function () {
        if ($(this).data('mode') === 'range') {
            $(this).flatpickr({
                static     : true,
                minDate    : $(this).data('mindate'),
                defaultDate: [$(this).data('defaultdatestart'), $(this).data('defaultdateend')],
                dateFormat : $(this).data('dateformat'),
                altInput   : true,
                altFormat  : $(this).data('altformat'),
                mode       : 'range',
                locale     : $(this).data('locale'),
                conjunction: '-',
                onOpen     : function () {
                    if (!$('.ui.modal.active').hasClass('scrolling')) {
                        $('.ui.modal.active').addClass('scrollable-by-fpickr');
                    }
                    $('.ui.modal.active').addClass('scrolling');
                    $('body').addClass('scrolling');
                },
                onClose     : function () {
                    if ($('.ui.modal.active').hasClass('scrollable-by-fpickr')) {
                        $('.ui.modal.active').removeClass('scrolling scrollable-by-fpickr');
        ...
        $modal
            .uiModal({
                ...
                onApprove : function ($clicked) {
                    ...
                    var coupon_validity_date_range = $modal.find('input[name="validity_date_range"]').val();
                    ...
                    $.post(
                        TB_vars.post_url,
                        {
                            ...
                            coupon_validity_date_range: coupon_validity_date_range,
        ...
        $modal
            .uiModal({
                ...
                onApprove : function ($clicked) {
                    ...
                    var campaign_validity_date_range = $modal.find('input[name="validity_date_range"]').val();
                    var promotion_bound_start = $modal.find('input[name="bound_start_date"]').val();
                    var promotion_bound_end = $modal.find('input[name="bound_end_date"]').val();
                    var bound_start_active = $modal.find('input[name="bound_start_date_active"]')[0].checked;
                    var bound_end_active = $modal.find('input[name="bound_end_date_active"]')[0].checked;
                    ...
                    $.post(
                        TB_vars.post_url,
                        {
                            ...
                            campaign_validity_date_range: campaign_validity_date_range,
        ...
        $modal
            .uiModal({
                ...
                onApprove : function ($clicked) {
                    ...
                    var promotion_validity_date_range = $modal.find('input[name="validity_date_range"]').val();
                    var promotion_bound_start = $modal.find('input[name="bound_start_date"]').val();
                    var promotion_bound_end = $modal.find('input[name="bound_end_date"]').val();
                    ...
                    $.post(
                        TB_vars.post_url,
                        {
                            ...
                            promotion_validity_date_range: promotion_validity_date_range,
    ...
    $('.tbk-apply-to-all').click(function (e) {
