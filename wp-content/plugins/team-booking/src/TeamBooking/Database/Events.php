    ...
    private static function map(\stdClass $db_object, $min_time, $max_time)
    {
        if (empty($db_object->recurrence) || $db_object->recurrence === 'N;' || !empty($db_object->recurring_event_id)) {
            ...
            foreach ($recurrence as $rule) {
				...
				// Fixes a possible Google Calendar bug
                $until_fix = strpos($rule, 'UNTIL=');
                if ($until_fix !== FALSE && $tail_format !== 'Ymd' && $rule[ $until_fix + 14 ] === ';') {
                    $sub  = substr($rule, $until_fix + 6, 8);
                    $rule = str_replace($sub, $sub . 'T235959Z', $rule);
                }
