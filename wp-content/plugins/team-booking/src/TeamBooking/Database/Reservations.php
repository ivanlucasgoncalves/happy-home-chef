...
class Reservations
{
     ...
     * @return \TeamBooking_ReservationData[]
     */
    public static function getByOrderId($order_id, $cache = TRUE)
    {
        if ($cache && NULL !== Cache::get('reservations' . $order_id)) {
    ...
    private static function getInstance($obj)
    {
        ...
        $tmp_obj = static::decode_object($obj->form_fields);
        if (is_array($tmp_obj)) {
            $reservation->setFormFields($tmp_obj);
        }
        ...
        $tmp_obj = static::decode_object($obj->discounts);
        if (is_array($tmp_obj)) {
            $reservation->setDiscount($tmp_obj);
        }
        ...
        $tmp_obj = static::decode_object($obj->payment_details);
        if (is_array($tmp_obj)) {
            $reservation->setPaymentDetails($tmp_obj);
        }
        $tmp_obj = static::decode_object($obj->files);
        if (is_array($tmp_obj)) {
            $reservation->setFileReference($tmp_obj);
        }
