...
/**
 * Class TeamBooking_Promotions_Coupon
 *
 * @author VonStroheim
 */
class TeamBooking_Promotions_Coupon implements \TeamBooking\Promotions\Promotion
{
    private $name;
    private $services;
    private $discount;
    ...
    {
        $code = \TeamBooking\Functions\tb_mb_strtolower($code);
        if (count($this->getList()) > 0) {

            return in_array($code, array_map('\\TeamBooking\\Functions\\tb_mb_strtolower', $this->getList()));
        }

        return $code === \TeamBooking\Functions\tb_mb_strtolower($this->getName());
    }
}