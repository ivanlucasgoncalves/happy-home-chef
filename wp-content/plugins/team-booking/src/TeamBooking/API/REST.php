...
class REST
{
    ...
    private function GET()
    {
        switch ($this->params['operation']) {
            ...
            case 'cancel':
                $reservation = $this->scopeReservation();
				$page        = Functions\getSettings()->getLinksDirectPage();
                if ($page && $this->params['show_confirm'] && get_post($page)) {
                    wp_redirect(get_page_link($page) . '?id=' . $reservation->getDatabaseId(TRUE) . '&token=' . $reservation->getToken(), 301);
                    exit;
                }
                if (!$reservation) {
                        ...
                        if ($this->params['show_confirm']) {
							...
							if ($page && get_post($page)) {
                                wp_redirect(get_page_link($page)
                                    . '?id=' . $reservation->getDatabaseId(TRUE)
                                    . '&token=' . $reservation->getToken() . '&success=1', 301);
                                exit;
                            }
                            $this->integrateWithTheme(
                                '<h2>'
