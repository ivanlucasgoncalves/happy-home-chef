...
class TeamBooking_PaymentGateways_Stripe_Settings implements TeamBooking_PaymentGateways_Settings
{
    ...
    public function prepareGateway(array $items, $additional_parameter = NULL)
    {
        ...
        $stripe->setRedirect($this->redirect_url);
    ...
    public function saveBackendSettings(array $settings, $new_currency_code)
    {
        if (isset($settings['use_gateway'])) {
            ...
            try {
                $url = admin_url() . 'admin-ajax.php?action=teambooking_stripe_webhook';
                $endpoint = \Stripe\WebhookEndpoint::create(array(
                    'url'            => $url,
                    'enabled_events' => array('checkout.session.completed')
                ));

                $this->webhook_id     = $endpoint->id;
                $this->webhook_secret = $endpoint->secret;
            } catch (Exception $e) {
            }
        }
    ...
    public function getBackendSettingsTab()
    {
        ...
        $element->addDescription(__('After Stripe payment, the customer will be redirected to this URL.', 'team-booking'));
    ...
    public function listenerIPN($post_data)
    {
        ...
        try {
            $event = \Stripe\Webhook::constructEvent(
                $post_data, $sig_header, $this->webhook_secret
            );
        ...
        if ($event->type === 'checkout.session.completed') {
            ...
            try {
                $pi       = \Stripe\PaymentIntent::retrieve($checkoutSessionCompleted->payment_intent);
                $charge   = \Stripe\Charge::constructFrom($pi->charges->data[0]);
                $order_id = $pi->metadata['tbk_order_id'];
            } catch (\Stripe\Error\Base $e) {
                error_log($e->getMessage());
                exit;
            }
            $reservation        = \TeamBooking\Database\Reservations::getByToken($order_id);
            ...
            if (!$reservation && empty($reservations_order)) { // Reservation is not found!
                error_log('Reservation token not found: ' . $order_id);
                exit;
            }
            ...
            $payment_details_array['transaction id'] = $pi->id;
			
			$currency                             = TeamBooking\Toolkit\getCurrencies(Functions\getSettings()->getCurrencyCode());
            $multiplier                           = $currency['decimal'] === 0 ? 1 : (10 ** $currency['decimal']);
            $payment_details_array['paid amount'] = $pi->amount / $multiplier;
            ...
            foreach ($reservations as $reservation) {
                ...
                if (!$reservation->isPending()) {
                    ...
                    \TeamBooking\Database\Reservations::update($reservation);
                } else {
