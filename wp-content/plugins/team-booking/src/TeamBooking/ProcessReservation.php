...
class ProcessReservation
{
    ...
    public static function submitPayment(\TeamBooking_ReservationData $reservation_data = NULL, $gateway_id = NULL, $return = FALSE)
    {
        ...
        if (Functions\getSettings()->getPaymentGatewaySettingObject($gateway_id)->isOffsite()) {
            ...
            if ($gateway_id === 'stripe') {
                $response['status']            = 'stripe';
                $response['stripe_session_id'] = $redirect;
            }
        } else {
    ...
    public static function submitReservation($reservation_data = NULL, $to_frontend = TRUE, $who = NULL)
    {
        ...
        if ($service->getSettingsFor('payment') === 'immediately'
            ...
            $resList = Database\Reservations::getByCoworker($reservation_data->getCoworker());
            foreach ($resList as $res) {
                if ($res->getGoogleCalendarEvent() === $reservation_data->getGoogleCalendarEvent()
                    && $res->getGoogleCalendarEventParent() === $reservation_data->getGoogleCalendarEventParent()
                    && $service->getClass() === 'appointment'
                    && !Functions\isReservationTimedOut($res)
                ) {
                    $error = new \TeamBooking_Error(2);
                    if (!$to_frontend) return $error;
                    Toolkit\ajaxJsonResponse(array(
                        'status'  => 'error',
                        'content' => self::getErrorMessage($error->getMessage())
                    ));
                    $res->getDatabaseId();
                }
            }
