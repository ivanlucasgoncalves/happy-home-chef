...
class TeamBooking_ReservationData
{
    ...
    public function getHooksArray($customer_timezone = FALSE)
    {
        ...
        // Important: for the following to output a correct value, reservations must be already in db
        if ($this->getOrderId()) {
            $reservations_order = Database\Reservations::getByOrderId($this->getOrderId(), FALSE);
            $order = new \TeamBooking\Order();
            $order->setItems($reservations_order);
            $hooks_values_array['total_order_price'] = Functions\currencyCodeToSymbol($order->get_full_amount(), $this->getCurrencyCode());
        }
