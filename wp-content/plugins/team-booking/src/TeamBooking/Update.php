...
class Update
{
    ...
    private static function doUpdate()
    {
        ...
        if ((!defined('DOING_AJAX') || !DOING_AJAX) && (!defined('DOING_CRON') || !DOING_CRON)) {
			...
			if (version_compare($stored_version, '2.5.7', '<')) {
                $settings = self::from2_5_7($settings);
            }
	...
	private static function from2_5_7(\TeamBookingSettings $settings){
        global $tbk_stripe_calling;
        $tbk_stripe_calling = TRUE;
        $gateway = Functions\getSettings()->getPaymentGatewaySettingObject('stripe');
        \Stripe\Stripe::setApiKey($gateway->getSecretKey());
        try {
            ...
            $gateway->setWebhookId($endpoint->id);
            $gateway->setWebhookSecret($endpoint->secret);
        } catch (\Exception $e) {
        }
        return $settings;
    }
