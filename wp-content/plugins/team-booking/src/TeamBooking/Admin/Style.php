...
class Style
{
    ...
    private function getTemplateVPages()
    {
		...
		// Direct page
        $element = new Framework\PanelSettingSelector(__('Direct Page', 'team-booking'));
        $element->addFieldname('tb-direct-page');
        $element->addNotice(__('If you delete the selected page, remember to check this setting again!', 'team-booking'));
        $element->addDescription(__('If for some reasons the cancellation or approval page rendered by the plugin does not look good with your theme, create a new page, put the [tb-cancellation] or [tb-approval] shortcode inside it, then select here such page.', 'team-booking'));
        $element->setSelected(Functions\getSettings()->getLinksDirectPage());
        $element->addOption(0, __('No Direct Page', 'team-booking'));
        $args  = array(
            'sort_order'   => 'asc',
        ...
        foreach ($pages as $page) {
            $element->addOption($page->ID, $page->post_title);
        }
        $element->appendTo($panel);
