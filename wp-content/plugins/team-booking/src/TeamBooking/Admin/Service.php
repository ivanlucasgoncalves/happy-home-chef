...
class Service
{
    ...
    private function getGCalSettingsContent()
    {
        ...
        if ($this->service->getClass() !== 'unscheduled') {
			...
			$element->addOption('PT4H', sprintf(_n('%d hour before', '%d hours before', 4, 'team-booking'), 4));
            $element->addOption('PT8H', sprintf(_n('%d hour before', '%d hours before', 8, 'team-booking'), 8));
        ...
        if ($this->service->getClass() !== 'unscheduled') {
            ...
            if ($this->service->getClass() === 'event') {
                ...
                $element->addOption(array(
                    'label'       => __('Use the content of the notification e-mail as event description', 'team-booking'),
                    'value'       => 1,
                    'checked'     => $custom_booking_type_settings->getEventDescriptionContent() === 1
                ));
            }
