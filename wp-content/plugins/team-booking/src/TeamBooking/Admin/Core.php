...
class Core
{
    ...
    private function getSettings()
    {
		...
		$warning_message = __('Warning: some reservations will be deleted on save', 'team-booking');
        $element->addOption(15 * DAY_IN_SECONDS, sprintf(_n('%d day', '%d days', 15, 'team-booking'), 15));
        if (Functions\cleanReservations(FALSE, TRUE, 15 * DAY_IN_SECONDS)) $element->addWarning(15 * DAY_IN_SECONDS, $warning_message);
        $element->addOption(30 * DAY_IN_SECONDS, sprintf(_n('%d day', '%d days', 30, 'team-booking'), 30));
        if (Functions\cleanReservations(FALSE, TRUE, 30 * DAY_IN_SECONDS)) $element->addWarning(30 * DAY_IN_SECONDS, $warning_message);
        $element->addOption(60 * DAY_IN_SECONDS, sprintf(_n('%d day', '%d days', 60, 'team-booking'), 60));
        if (Functions\cleanReservations(FALSE, TRUE, 60 * DAY_IN_SECONDS)) $element->addWarning(60 * DAY_IN_SECONDS, $warning_message);
        $element->addOption(120 * DAY_IN_SECONDS, sprintf(_n('%d day', '%d days', 120, 'team-booking'), 120));
        if (Functions\cleanReservations(FALSE, TRUE, 120 * DAY_IN_SECONDS)) $element->addWarning(120 * DAY_IN_SECONDS, $warning_message);
        $element->addOption(240 * DAY_IN_SECONDS, sprintf(_n('%d day', '%d days', 240, 'team-booking'), 240));
        if (Functions\cleanReservations(FALSE, TRUE, 240 * DAY_IN_SECONDS)) $element->addWarning(240 * DAY_IN_SECONDS, $warning_message);
		$element->addOption(1 * YEAR_IN_SECONDS, sprintf(_n('%d year', '%d years', 1, 'team-booking'), 1));
        if (Functions\cleanReservations(FALSE, TRUE, 1 * YEAR_IN_SECONDS)) $element->addWarning(1 * YEAR_IN_SECONDS, $warning_message);
        $element->addOption(2 * YEAR_IN_SECONDS, sprintf(_n('%d year', '%d years', 2, 'team-booking'), 2));
        if (Functions\cleanReservations(FALSE, TRUE, 2 * YEAR_IN_SECONDS)) $element->addWarning(2 * YEAR_IN_SECONDS, $warning_message);
        $element->addOption(3 * YEAR_IN_SECONDS, sprintf(_n('%d year', '%d years', 3, 'team-booking'), 3));
        if (Functions\cleanReservations(FALSE, TRUE, 3 * YEAR_IN_SECONDS)) $element->addWarning(3 * YEAR_IN_SECONDS, $warning_message);
        $element->addOption(0, __('Forever', 'team-booking'));
        $element->appendTo($panel);
